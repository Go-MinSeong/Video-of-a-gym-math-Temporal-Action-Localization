...
import random
from utils import right_proposal, cal_iou, save_window, compute_iou, save_window_v3, video_info, nms_v2


def continuous_random(x, y):
    num=random.randrange(x,y-7)
    return num,num+8
def features_to_proposal(vector, window_size, start):
    ...
    for length in window_lengths:
        ...
        for i in range(num_windows):
            if length ==8:
                ...
                num = continuous_random(start+i*stride,start+i*stride+length) 
                v = vector[num[0]:num[1], :]
                if np.shape(v)!=(8, 2048):
                    #print(num, np.shape(v))
                    continue;
...
def writing(fl, lst, data, name, start, end, window_size):
    ...
    #for w in lst:
    #    print(start, w[1], end)
    st = save_window_v3(lst, data["databases"][name], name)
...
def main(model, f, data, dir_home, window_size):
    ...
    for win in tqdm(s_window):
        ...
        proposal_list = features_to_proposal(vector, window_size, start)
        ...
        writing(fl, lst, data, f.split(".")[0], start, end, window_size)
...
if __name__=="__main__":
    ...
    for fe in tqdm(f_lst):
        print(fe, "start")
        main(model, fe, data, dir_home, 400)

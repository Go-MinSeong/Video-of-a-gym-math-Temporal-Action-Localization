...
import random
from utils import right_proposal, cal_iou, save_window10
import pickle
import random
import time
import torch
import torch.nn.functional as F
# train :  120 , validation :  31 으로 스플릿 진행합니다.
def continuous_random(x, y):
    num=random.randrange(x,y-7)
    return num,num+8

def main(dir_home = "/home/work/TAL_FineGym/Feature_RGB", only="1", thresh_hold=0.2):
    ...
    for k, f in tqdm(enumerate(features)):
        ...
        window_lengths = [8, 12, 16, 24, 32, 64, 96, 128]  # sliding window의 길이
            ...
            for l in lst_1: # l [12, 23]
                ...
                for i in range(num_windows):
                    ...
                    if length == 8:
                        ...
                        v = vector[l_0+i*stride:l_0+i*stride+length, :]
                        v = torch.tensor(v)
                        v = np.array(F.adaptive_max_pool1d(v.unsqueeze(0), 8).squeeze(0))
                        if np.shape(v)!=(8, 2048):
                            continue;
                        window_list.append((v, [l_0+i*stride,l_0+i*stride+length], f.split(".")[0]))
        ...
        if subset=="train": # train dataset
            tresult_lst.extend(save_window10(window_list, data["databases"][f.split(".")[0]])[0])
            tresultL_lst.extend(save_window10(window_list, data["databases"][f.split(".")[0]])[1])

        elif subset=="val": # validation dataset
            vresult_lst.extend(save_window10(window_list, data["databases"][f.split(".")[0]])[0])
            vresultL_lst.extend(save_window10(window_list, data["databases"][f.split(".")[0]])[1])
